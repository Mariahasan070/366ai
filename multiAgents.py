...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        """
        Returns the best action using alpha-beta pruning, self.depth, and self.evaluationFunction.
        """
        print("Running getAction for AlphaBetaAgent...")  # Debug statement
        action, _ = self.alphaBeta(0, 0, gameState, float('-inf'), float('inf'))
        print(f"Selected action: {action}")  # Debug statement
        return action

    def alphaBeta(self, curr_depth, agent_index, gameState, alpha, beta):
        """
        Alpha-beta pruning implementation that evaluates the game tree.
        """
        if curr_depth == self.depth or gameState.isWin() or gameState.isLose():
            # Return evaluation if max depth is reached or game ends
            return None, self.evaluationFunction(gameState)

        if agent_index == 0:  # Maximizing for Pacman (Agent 0)
    ...
    def maxValue(self, curr_depth, agent_index, gameState, alpha, beta):
        """
        Computes the maximum value for Pacman, pruning branches when possible.
        """
        best_action = None
        ...
        for action in gameState.getLegalActions(agent_index):
            ...
            _, score = self.alphaBeta(curr_depth + 1, (agent_index + 1) % gameState.getNumAgents(), successor, alpha, beta)
            ...
            # Alpha-beta pruning
            if best_score >= beta:
                print(f"Pruning: Best score {best_score} >= beta {beta}")  # Debug statement
                return best_action, best_score  # Beta cutoff
    ...
    def minValue(self, curr_depth, agent_index, gameState, alpha, beta):
        """
        Computes the minimum value for Ghosts, pruning branches when possible.
        """
        best_action = None
        best_score = float('inf')
        ...
        for action in gameState.getLegalActions(agent_index):
            ...
            # Alpha-beta pruning
            if best_score <= alpha:
                print(f"Pruning: Best score {best_score} <= alpha {alpha}")  # Debug statement
                return best_action, best_score  # Alpha cutoff
